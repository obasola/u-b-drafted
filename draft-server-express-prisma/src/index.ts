...
app.get("/combine_scores", async function (req: Request, res: Response) {
  const combineScores = await AppDataSource.getRepository(CombineScore).find();
...
app.delete("/pick/:id", async function (req: Request, res: Response) {
  const results = await AppDataSource.getRepository(Pick).delete(req.params.id);
  return res.send(results);
});
...
app.post("/schedule", async function (req: Request, res: Response) {
  const schedule = await AppDataSource.getRepository(Schedule).create(req.body);
